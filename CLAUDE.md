...
- preserve assumptions and evidence as append-only canonical records. Historical decision snapshots may preserve retired `DNB###` and `T###` guardrails, but a historical reference never creates or substitutes for a missing `A###` assumption or `E###` evidence record.
- link an experiment execution action to its concrete preregistered `X###`; draft, completed, and cancelled experiments cannot be presented as the current executable action. A decision that follows a completed experiment starts from the gate outcome its locked decision rule assigns to `results.branch`, and records any departure from it. Cancelled drafts may remain unregistered when their cancellation timestamp and reason are preserved.
...
- in a forked run, do not open another run under `evals/runs/` — its parent or any other fork — nor `evals/runs/.keys/`;
